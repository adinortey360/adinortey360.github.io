$.ajax({
    ...
    content.projects.forEach(element => {
      $("#projects").append('<div class="resume-row col-md-5"> <div class="row"> <div class="col-md-12 col-xl-12"> <div> <h6>'+element.name+'</h6><label>'+element.category+' | '+element.status+'</label>  <p>'+element.description+'</p> <a href="'+element.link+'" target="_blank" style="color: #fff;text-decoration: underline;">See Project</div> </div> </div> </div> </div>');
    });
